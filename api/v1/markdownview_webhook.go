...
//+kubebuilder:webhook:path=/mutate-view-bobuhiro11-net-v1-markdownview,mutating=true,failurePolicy=fail,sideEffects=None,groups=view.bobuhiro11.net,resources=markdownviews,verbs=create;update,versions=v1,name=mmarkdownview.kb.io,admissionReviewVersions=v1
...
//+kubebuilder:webhook:path=/validate-view-bobuhiro11-net-v1-markdownview,mutating=false,failurePolicy=fail,sideEffects=None,groups=view.bobuhiro11.net,resources=markdownviews,verbs=create;update,versions=v1,name=vmarkdownview.kb.io,admissionReviewVersions=v1
...
func (r *MarkdownView) validate() (admission.Warnings, error) {
	...
	if r.Spec.Replicas < 1 || r.Spec.Replicas > 5 {
		errs = append(errs,
			field.Invalid(
				...
				r.Spec.Replicas,
				"replicas must be in the range of 1 to 5.",
			),
		)
	}
	...
	if !hasSummary {
		errs = append(errs,
			field.Required(
				field.NewPath("spec", "markdowns"),
				"markdowns must have SUMMARY.md.",
			),
		)
	}
	...
	if len(errs) > 0 {
		err := apierrors.NewInvalid(
			schema.GroupKind{Group: GroupVersion.Group, Kind: "MarkdownView"},
			r.Name,
			errs,
		)
